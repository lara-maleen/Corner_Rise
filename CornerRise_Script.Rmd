...
process_dive <- function(dive_df) {
  ...
  dplyr::mutate(
   ...
   dplyr:: mutate(cum_distance_m = cumsum(segment_distance_m)) %>%
    ungroup()
}
...
# Count total observations for single-dive morphotypes
abundance_single_dive <- data.a %>%
  filter(Morphotype %in% morphotypes_single_dive$Morphotype) %>%
  dplyr::group_by(Morphotype) %>%
  dplyr::summarise(TotalObservations = n())  # each row = 1 observation

# Optional: identify true singletons
singleton_morphotypes <- abundance_single_dive %>%
  filter(TotalObservations == 1)

# Print results
knitr::kable(abundance_single_dive, caption = "Observations of morphotypes recorded at a single dive location")
...
SPE2_matrix <- xtabs(~ dive_name + Morphotype, data = data.a)
...
df_summary <- data.a %>%
  dplyr::group_by(depth, Phylum, dive_name) %>%
  dplyr::summarise(count = n(), .groups = "drop")

# Merge dive track length with df_summary
df_corrected <- df_summary %>%
  dplyr::left_join(corrected_summary, by = "dive_name") %>% 
  dplyr::mutate(transect_m = as.numeric(total_track_length_no_jumps), #thats in meters, accounts for the off=bottom time
...
df_corrected <- df_corrected %>%
  dplyr::mutate(depth_bin = cut(depth, breaks = seq(0, max(depth), by = 5), include.lowest = TRUE)) %>%
  dplyr::mutate(depth_bin = as.factor(depth_bin))  
...
## Do not effort correct but I cant correct with linear method. Instead use uncorrected raw data / morphotype counts.
...
# SEE THE SAC ANALYSIS - We cannot compare morphotype diversity or richness across dives because we dont have sufficient sampling... 
...
peak_density <- df_corrected %>%
  ...
  ungroup() %>%
  select(dive_name, Phylum, depth, Effort_Corrected_count)
...
#Load CTD data for complete surface to bottom track of ROV to plot the profile (down to 4000m in Dive 5). I had the raw files for this (.hex and .xmlcon) which needed to be converted to .cnv - I did not do this, as a specific software is needed for this. But once this is done, proceed with plotting. 

ctd <- read.ctd('/Users/lara/Downloads/Corner Rise Video Analysis/raw_data/ex2104-DIVE05-ROV-CTD/EX2104_DIVE05_20210708_ROVCTD.cnv')
...
#Need to convert Oxygen to mg/L as in my other plots (error <1%). Oxygen2 is ml/L 
ctd.data$oxygen_mgL <- ctd.data$oxygen2 * 1.429

#The sensor broke or measured wrong starting at the bottom - so I need to exclude salinity values starting from reaching to bottom

max_depth <- max(ctd.data$depth, na.rm = TRUE)
bottom_cutoff <- max_depth - 50   # I'm cutting the last 50m off, so I avoid the faulty sensor readings. 

ctd.data <- ctd.data %>%
  filter(depth < bottom_cutoff)

ctd.data <- ctd.data %>%
  filter(
    salinity > 25 & salinity < 38
  #  oxygen_mgL > 0 & oxygen_mgL < 8.4 #do not include, this will cut off the surface water measuremenets, just removing the faulty salinity rows is enough
)
...
ggplot(ctd.data, aes(x = oxygen2, y = temperature, color = depth)) +
  geom_point(size = 1) +
  scale_color_viridis(option = "viridis", direction = -1) +
  xlab("Oxygen (ml/L)") +
  ylab("Temperature (°C)") +
...
# Compute sigma-theta
ctd.data$sigmaT <- swSigmaTheta(ctd.data$salinity,
                                ctd.data$temperature,
                                ctd.data$pressure)

# Define LSW sigma-theta limits (Henry et al.)
lsw_min <- 27.780
lsw_max <- 27.834

# Filter rows in that density range
lsw_rows <- subset(ctd.data, sigmaT >= lsw_min & sigmaT <= lsw_max)
lsw_depths <- lsw_rows$depth
lsw_salinity<- lsw_rows$salinity
max(lsw_salinity)
min(lsw_salinity)

lsw_oxy<- lsw_rows$oxygen_mgL
max(lsw_oxy)
min(lsw_oxy)

lsw_temp<- lsw_rows$temperature
max(lsw_temp)
min(lsw_temp)

# Create T–S grid for density contours
sal_grid <- seq(min(ctd.data$salinity, na.rm=TRUE), max(ctd.data$salinity, na.rm=TRUE), length.out=100)
temp_grid <- seq(min(ctd.data$temperature, na.rm=TRUE), max(ctd.data$temperature, na.rm=TRUE), length.out=100)
grid <- expand.grid(salinity = sal_grid, temperature = temp_grid)
grid$sigmaT <- swSigmaTheta(grid$salinity, grid$temperature, 0)  # reference 0 dbar

# Optional: depth profile with LSW range
ggplot(ctd.data, aes(x = temperature, y = depth)) +
  geom_line() +
  geom_hline(yintercept = range(lsw_depths), linetype="dashed", color="red") +
  scale_y_reverse() +
  ylab("Depth (m)") +
  xlab("Temperature (°C)")

# Create T–S plot with labeled sigma-theta contours
ggplot(ctd.data, aes(x = salinity, y = temperature, color = depth)) +
  geom_point(size = 1) +
  # grey density lines + labels
  geom_contour(data = grid, aes(z = sigmaT, x = salinity, y = temperature),
               color = "grey60", bins = 20) +
  # highlight LSW range in red
  geom_contour(data = grid, aes(z = sigmaT, x = salinity, y = temperature),
               color = "red", breaks = c(lsw_min, lsw_max), linetype = "dashed") +
  geom_text_contour(data = grid, aes(z = sigmaT, x = salinity, y = temperature),
                    breaks = lsw_min, color = "red", size = 3,
                    label.placer = label_placer_flattest(), nudge_y = 0.13) +
   geom_text_contour(data = grid, aes(z = sigmaT, x = salinity, y = temperature),
                    breaks = lsw_max, color = "red", size = 3,
                    label.placer = label_placer_flattest(), nudge_y = -0.13) +
  scale_color_viridis(option = "viridis", direction = -1) +
  xlab("Salinity (PSU)") +
  ylab("Temperature (°C)") +
...
plot_data <- ctd.data %>%
  select(pressure, temperature, salinity, oxygen_mgL) %>%
...
ggplot(data.clean %>% filter(depth > 900), aes(y = temp, x = oxygen, color = depth)) +
  geom_point(size = 1) +
  scale_color_viridis(option = "viridis", direction = -1, limits = depth_limits) +
  xlab("Oxygen (mg/L)") +
  ylab("Temperature (°C)") +
...
ggplot(data.clean, aes(x = psu, y = temp, color = depth)) +
  geom_point(size = 1) +
  scale_color_viridis(
    option = "viridis",
    direction = -1,
    limits = depth_limits,           # shared limits
    name = "Depth (m)"               # cleaner legend title
  ) +
  xlab("Salinity (PSU)") +
  ylab("Temperature (°C)") +
  theme_linedraw() +
...
#CHeck how different the cluster 1 is to cluster 2 (basically calculating the bray curtis dissimilarity between clusters)

bc <- vegdist(community_matrix, method = "bray")
group1 <- which(group_df$group == "cluster1")
group2 <- which(group_df$group == "cluster2")

pairwise_bc <- as.matrix(bc)[group1, group2]
overall_cluster1_cluster2 <- mean(pairwise_bc)
overall_cluster1_cluster2 #cluster 1 is the deeper sites, cluster 2 the shallow sites
...
> SACs for the gardens

```{r}


# 1. Create garden × morphotype matrix
garden_matrix <- xtabs(count ~ garden_id + morphotype, data = garden_long)

# 2. Convert to list format (each element = abundances per garden)
abund_list_garden <- apply(garden_matrix, 1, function(x) x[x > 0])

# 3. Individual SAC plots per garden
for (g in names(abund_list_garden)) {
  cat("Processing", g, "...\n")
  out_g <- iNEXT(abund_list_garden[[g]], q = 0, datatype = "abundance")
  
  suppressWarnings({
    p <- ggiNEXT(out_g, type = 1) +
      ggtitle(paste("Species Accumulation Curve -", g)) +
      theme_minimal()
    print(p)
  })
}
...
# 4. Combined SAC plot for all gardens
out_all_g <- iNEXT(abund_list_garden, q = 0, datatype = "abundance")

p_all_g <- ggiNEXT(out_all_g, type = 1, color.var = "Assemblage") +
  geom_line(size = 0.3) +
  theme_minimal() +
  labs(
    title = "Species Accumulation Curves per Garden",
    x = "Number of individuals",
    y = "Species richness (q = 0)",
    color = "Garden"
  ) +
...
garden_long <- garden_long %>%
  mutate(count_bin = cut(
    count,
    breaks = c(0, 5, 20, 50, 1e6),  # last bin covers all larger counts
    labels = c("0-5", "6-20", "21-50", "50+"),
    right = TRUE,
    include.lowest = TRUE
  ))
...
ggplot() +
  # LOESS path
  geom_smooth(
    data = expanded_garden_paths, 
    aes(x = lon, y = lat, group = garden), 
    method = "loess", span = 0.5, se = FALSE, 
    color = "grey91", linewidth = 3
  ) +
  # Points with binned sizes
  geom_point(
    data = garden_long, 
    aes(x = lon, y = lat, size = count_bin, color = Phylum),
    position = position_jitter(width = 0.0001, height = 0.0001),
    alpha = 0.5
  ) +
  scale_size_manual(
    values = c(2, 6, 10, 14), # visual sizes for each bin
    name = "Count range"
  ) +
  facet_wrap(~ garden, scales = "free") +
  theme_minimal()
